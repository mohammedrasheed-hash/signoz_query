...
export default function App() {
  ...
  const [env, setEnv] = useState({ prod: false, poc: true });
  const [harFile, setHarFile] = useState(null);
  const [ticketKeywords, setTicketKeywords] = useState("");
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (harFile) fd.append("har", harFile);
      if (ticketKeywords.trim()) fd.append("ticket_keywords", ticketKeywords.trim());
    ...
    <div className="page">
      ...
      <div className="grid">
        ...
        <section className="panel">
          ...
          <h2 className="eyebrow eyebrow--spaced">Ticket keywords</h2>
          <label className="field">
            <span>
              Optional, comma-separated <em className="muted">— e.g. terms an LLM
              pulled out of the ticket text</em>
            </span>
            <input
              value={ticketKeywords}
              onChange={(e) => setTicketKeywords(e.target.value)}
              placeholder="e.g. QuickAddIntel, report description"
            />
          </label>
          ...
          {result && (
            <>
              ...
              {result.candidates && result.candidates.length > 0 && (
                <>
                  <h2 className="eyebrow eyebrow--spaced">
                    All candidate queries ({result.candidates.length})
                  </h2>
                  {result.candidates.map((c, i) => (
                    <div className="out" key={i}>
                      <div className="out-head">
                        <span>
                          <span className="tag">{c.strategy}</span> {c.description}
                        </span>
                        <button
                          className="copy"
                          onClick={() => copy(c.query_without_time, `cand-${i}`)}
                          type="button"
                        >
                          {copied === `cand-${i}` ? "✓ copied" : "copy"}
                        </button>
                      </div>
                      <code>{c.query_without_time}</code>
                    </div>
                  ))}
                </>
              )}

              <h2 className="eyebrow eyebrow--spaced">Best guess (single query)</h2>
              <div className="out">
                <div className="out-head">
                  <span>Base only — no body filters, no time</span>
                  <button
